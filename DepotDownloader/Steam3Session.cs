...
namespace DepotDownloader
{
    public class Steam3Session
    {
        ...
        public IAuthenticator Authenticator = new ConsoleAuthenticator();
        ...
        public async Task<ulong> GetDepotManifestRequestCodeAsync(uint depotId, uint appId, ulong manifestId, string branch)
        {
            ...
            if (requestCode == 0)
            {
                WriteLog($"No manifest request code was returned for depot {depotId} from app {appId}, manifest {manifestId}");
        ...
        private void DisconnectedCallback(SteamClient.DisconnectedCallback disconnected)
        {
            ...
            else if (!bAborted)
            {
                ...
                if (bConnecting)
                {
                    WriteLog($"Connection to Steam failed. Trying again (#{connectionBackoff})...");
                }
                else
                {
